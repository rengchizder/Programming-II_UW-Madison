...
public class ExceptionalLibrary {
  ...
  /**
   * Parses the String argument as a PIN code, PIN must be integer between 1000 and 9999, inclusive
   * 
   * @param s           String that represents a PIN code
   * @param errorOffset errorOffset for the ParseException if thrown
   * @return an integer that represents the parsed PIN if it is valid (4-digits number in the range
   *         of [1000, 9999]
   * @throws ParseException if the String argument cannot be parsed as a PIN code
   */
  public int parsePinCode(String s, int errorOffset) throws ParseException {
    try {
      int i = Integer.parseInt(s); // try parse the String argument s as a PIN code
      if (i < 1000 || i > 9999) { // check if the integer number is in the range required
        throw new Exception("integer not in range");
      } else
        return i; // return if PIN is in range
    } catch (NumberFormatException e) { // syntax error
      throw new ParseException("Error: PIN must be integer between 1000 and 9999, inclusive.\n",
          errorOffset);
    } catch (Exception e) {
      throw new ParseException("Error: PIN must be integer between 1000 and 9999, inclusive.\n",
          errorOffset);
  ...
  /**
   * Parses the String argument as a card bar code
   * 
   * @param s           String that represents a card bar code
   * @param errorOffset errorOffset for the ParseException if thrown
   * @return an integer that represents the parsed card bar code if it is valid
   * @throws ParseException if the String argument cannot be parsed as a card bar code
   */
  public int parseCardBarCode(String s, int errorOffset) throws ParseException {
    try {
      int i = Integer.parseInt(s); // try parse the String argument s as a card bar code
      if (Subscriber.checkCardBarCode(i) != true) { // check if the card bar code is a correct bar
                                                    // code for the subscriber
        throw new ParseException("Error: Parsed card bar code not valid.\n", errorOffset);
      } else
        return i; // return if bar code is correct
    } catch (NumberFormatException e) { // syntax error
      throw new ParseException("Error: Parsed card bar code not valid.\n", errorOffset);
    }
  }
  ...
  /**
   * Parses the String argument as a book ID
   * 
   * @param s           String that represents a book ID
   * @param errorOffset errorOffset for the ParseException if thrown
   * @return an integer that represents the book id if valid
   * @throws ParseException if the String argument cannot be parsed as a book ID
   */
  public int parseBookId(String s, int errorOffset) throws ParseException {
    try {
      int i = Integer.parseInt(s); // try parse String argument s as a book ID
      return i;
    } catch (NumberFormatException e) { // syntax error
  ...
  public void parseRunLibrarianAddSubscriberCommand(String[] commands)
    ...
    int pin = parsePinCode(commands[2], 2); // parse pin code into integer
    parsePhoneNumber(commands[4], 4); // try parsing phone number into numbers
  ...
  public void parseRunLibrarianSaveBooksCommand(String[] commands) throws ParseException {
    this.checkCommandArgumentsCount(commands, 2);
    try {
      PrintWriter output = new PrintWriter(commands[1]);
      int i = 0;
      while (i < books.size()) {
      ...
      System.out.println("WARNING: File could not be opened/created for writing.");
    }
  }
  ...
  public void parseRunLibrarianLoadBooksCommand(String[] commands) throws ParseException {
    ...
    try {
      inFile = new BufferedReader(new FileReader(commands[1]));
      String line = null;
      while ((line = inFile.readLine()) != null) {
        try {
          splitStringAndAddBook(line);
        } catch (ArrayIndexOutOfBoundsException | NullPointerException e) {
          if (line.trim().isEmpty() == false) {
            System.out
                .println("Error: Found incorrectly formatted line in file books3.txt: " + line);
          }
        }
      }
    } catch (IOException e) {
      System.out.println("WARNING: File could not be opened for reading.");
    } finally {
      if (inFile != null) {
        try {
          inFile.close();
        } catch (IOException e) {
          System.out.println("WARNING: File could not be closed.");
        }
      }
    }
  }
  ...
  /**
   * helper method that split the string of title author and add the book into books
   * 
   * @param input the string containing title and author, separated by :
   */
  private void splitStringAndAddBook(String input) {
    String[] interOutput = input.split(":"); // from each line, split the scanner input line by : to
                                             // get title and author
    books.add(new Book(interOutput[0].trim(), interOutput[1].trim())); // add the book into books
                                                                       // list after trimming the
                                                                       // empty spaces
  }
