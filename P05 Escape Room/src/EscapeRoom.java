...
/**
 * This class models an escape room style adventure game. This game uses processing graphical
 * library.
 */
public class EscapeRoom extends PApplet { // this class inherits PApplet from the jar file
  // instance fields
  private PImage backgroundImage; // background image for the game
  private ArrayList<Thing> allThings; // array list of all the things present
  ...
  public static void main(String[] args) {
    PApplet.main("EscapeRoom"); // open the window for the game
  }
  ...
  public void settings() {
    size(800, 600); // set the size of window to 800 * 600.
  }

  /**
   * It's used to define initial environment properties such as loading the room, initializing the
   * array of things and initializing processing field for things
   */
  public void setup() {
    Thing.setProcessing(this); // initialize processing field for the things so that they can use
                               // PApplet methods.
    allThings = new ArrayList<Thing>(); // initialize new array of things.
    loadRoom("rooms" + File.separator + "computerCenter.room"); // load the new room
  }

  /**
   * It works like update method that will draw the image and call update on every Thing in the
   * list.
   */
  public void draw() {
    image(backgroundImage, 0, 0); // redraw the background image each time
    // iterate through the ArrayList and call update method on each one
    for (int i = 0; i < allThings.size(); i++) {
      Action pressedOn = allThings.get(i).update(); // initialize the action object
      if (pressedOn != null) { // if update return a non-null reference to an Action, call the act()
                               // method on such action
        pressedOn.act(allThings);
      }
    }
    // iterate through the ArrayList and remove Thing objects that are deactivated
    for (int i = 0; i < allThings.size(); i++) {
